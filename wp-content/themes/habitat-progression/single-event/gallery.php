...
if ( ! post_password_required() && metadata_exists( 'post', $post->ID, '_event_gallery' ) ) :
	...
	if ( ! empty( $event_gallery ) ) :
		...
		foreach ( $images as $image_id ) :
			...
			echo apply_filters( 'em_event_gallery_thumbnail_html', sprintf( '<a href="%s" class="event-thumbnail ' . implode( ' ', $classes ) . '" title="%s" data-rel="prettyPhoto[gallery-event]">%s</a>', $image_link, $image_title, $image ), $post->ID );
