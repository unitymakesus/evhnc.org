...
if ( ! post_password_required() && has_post_thumbnail() ) :
	...
	$size			= apply_filters( 'em_single_event_thumbnail_size', 'progression-events-single' );
	$attr			= apply_filters( 'em_single_event_thumbnail_attr', array( 'title' => $image_title ) );
	$image			= get_the_post_thumbnail( $post->ID, $size, $attr );

	echo apply_filters( 'em_single_event_thumbnail_html', sprintf( '<a href="%s" class="event-single-thumb post-thumbnail entry-thumbnail" title="%s" data-rel="prettyPhoto[gallery-event]">%s</a>', $image_link, $image_title, $image ), $post->ID );
